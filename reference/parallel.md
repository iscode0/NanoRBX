...
The guarantee is exact: every worker receives a seed, worker `i` receives `base + i`, and
the same base reproduces every worker's stream across a pool rebuild while a different base
